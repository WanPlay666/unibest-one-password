  ...
  () => {
    ...
    const setTokenInfo = (val: IAuthLoginRes) => {
      ...
      if (isSingleTokenRes(val)) {
        ...
        setStorage('accessTokenExpireTime', expireTime)
      }
      else if (isDoubleTokenRes(val)) {
        // 双token模式
        const accessExpireTime = now + val.accessExpiresIn * 1000
        const refreshExpireTime = now + val.refreshExpiresIn * 1000
        setStorage('accessTokenExpireTime', accessExpireTime)
        setStorage('refreshTokenExpireTime', refreshExpireTime)
      }
    }
    ...
    const isTokenExpired = computed(() => {
      ...
      const expireTime = getStorage('accessTokenExpireTime')
    ...
    const isRefreshTokenExpired = computed(() => {
      ...
      const now = nowTime.value
      const refreshExpireTime = getStorage('refreshTokenExpireTime')

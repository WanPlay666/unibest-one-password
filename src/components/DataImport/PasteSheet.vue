...
<template>
  <view v-if="show" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4"
    @click="handleClose">
    <view
      class="flex w-full max-w-[520px] max-h-[92vh] flex-col overflow-hidden rounded-3xl bg-[#1A1A1A] px-5 pb-6 pt-5"
      @click.stop>
      <text class="mb-3 block text-center text-sm text-white font-bold">粘贴 JSON</text>

      <textarea v-model="pasteText" :maxlength="-1"
        class="box-border w-full flex-1 min-h-[420px] rounded-[16px] border border-white/10 bg-[#050508] p-3 text-[11px] text-blue-400 font-mono"
        placeholder="将导出的 JSON 粘贴到这里..." placeholder-class="text-[11px] text-[#6b7280]" />
